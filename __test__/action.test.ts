...
describe('Action', () => {
    ...
    beforeAll(async () => {
        ...
        fakeUSDCToken = await createToken({ account: deployerAccount, name: 'USDC', algod });

        /** Opt In donator to USDC token */
        await optInToAsset({ account: donorAccount, assetIndex: fakeUSDCToken, algod })

        /** Opt In creator to USDC token */
        await optInToAsset({ account: actionCreatorAccount, assetIndex: fakeUSDCToken, algod })
    ...
    test('donorAccount has 10000 USDC', async () => {
        const accountInfo = await algod.accountAssetInformation(donorAccount.addr, fakeUSDCToken).do();
        expect(accountInfo['asset-holding'].amount).toBe(10000 * USDCBaseUnitMultiplier)
    });
    ...
    test.skip('Change token asset fails if not called by governance', async () => {
        await expect(appClient.changeTokenAsset({
    ...
    test('dispense fails if not called by action creator', async () => {
        ...
        await expect(appClient.dispense({
    ...
    test('dispense works correctly', async () => {
        ...
        const actionCreatorBalance = await algod.accountAssetInformation(actionCreatorAccount.addr, fakeUSDCToken).do();
        expect(actionCreatorBalance['asset-holding'].amount).toBe(withdrawAmount);

    });

    test('recollect fails if it not called 2 weeks after end date', async () => {

        await expect(appClient.recollect({
