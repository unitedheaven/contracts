...
fs.readdir(contractsDir, (err, files) => {
    ...
    files.filter(file => file.endsWith('.algo.ts')).forEach(file => {
        ...
        try {
            ...
            // Compile contract
            execSync(`tealscript contracts/${file} ${artifactsDir}/${baseName}`);

            // Generate client
            execSync(`algokitgen generate -a ${artifactsDir}/${baseName}/${contractClassName}.json -o ${clientsDir}/${baseName}.ts`);

            // Generate components
            execSync(`algokit-generate-component ${artifactsDir}/${baseName}/${contractClassName}.json ${componentsDir}/${baseName}`);
        } catch (error) {
